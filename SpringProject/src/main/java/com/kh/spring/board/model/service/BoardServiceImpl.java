...
public class BoardServiceImpl implements BoardService{
	...
	public void selectBoardList(int cp, Map<String, Object> map){
	...
	public void selectBoardKeywordList(int cp, Map<String, Object> map){
		...
		ArrayList<Board> list = boardDao.selectBoardKeywordList(pi, map);
		...
		if(boardNo > 0 && list != null) {
			...
			for(int i = 0; i<list.size();i++) {
				if(list.get(i).getSize() > 0) { // i번째 요소에 업로드된 이미지가 존재하는지 체크
					...
					// BoardImg객체를 생성해서 필드에 값을 세팅한후 boardImageList에 추가할 예정
			...
			// 분ㄹ류 작업 완료 후 boardImageList가 비어있는가 ? 등록한 이미지가 없음
			// 									비어있지않다 ? 등록한 이미지가 있음
			if(!boardImageList.isEmpty()) {
				...
				if(result == boardImageList.size()) {
					// 삽입된 행의 갯수와 업로드된 이미지 수가 같은 경우
					
					// 서버에 이미지를 저장.(Utils.savePath에서 선작업함)
			...
			if(list != null) {
				...
				if(result > 0) {
					...
					for( BoardImg img : boardImgList) {
						...
						// 6) 결과값이 0 => update에는 실패했고, 실제로 db에는 올라가야할 데이터이기때문에 insert시켜준다.
						if(result == 0) {
